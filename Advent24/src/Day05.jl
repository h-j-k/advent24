...
middlesum(updates) = map(update -> parse(Int, update[trunc(Int, (length(update) + 1) / 2)]), updates)

function part1(input)
    (rules, updates) = partitionby(input, r -> r == "")
    rulegroups = mergewith(union, map(rule -> let (k, v) = split(rule, "|"); Dict(k => Set([v])); end, rules)...)
    filter(iscorrect(rulegroups), map(update -> split(update, ","), updates)) |> middlesum |> sum
end
